...
- Compile production build: `npm run build` — always verify `dist/main.js` exists afterwards; pm2 runs that exact path ([ecosystem.config.js](file:///home/baxa/coding/e-commerse-back/ecosystem.config.js)). `tsconfig.build.json` pins `rootDir: "src"` and excludes `prisma`/`scripts`, because a stray `.ts` outside `src` raises the inferred root and silently relocates the entry point to `dist/src/main.js`. It also keeps `tsBuildInfoFile` **inside** `dist/`: left outside, `nest build` (which deletes `dist/`) leaves the incremental cache behind, tsc considers the build current, emits nothing, and exits 0 — a green build with no output at all.
...
- Write fiscal (IKPU) codes from [prisma/catalog/ikpu.json](file:///home/baxa/coding/e-commerse-back/prisma/catalog/ikpu.json) into the DB: `npm run db:set:ikpu` (`-- --dry-run` first). The codes live in a hand-edited file because they come from `tasnif.soliq.uz` and must be signed off by an accountant — the script only validates (17 digits, `vat_percent` present) and copies. A blank `ikpu_code` is skipped, leaving the DB value alone, so the file can be filled in stages. **Must run after `db:import:catalog`**, since `db:reset:catalog` deletes categories and the importer recreates them without fiscal fields.
...
- Mint a **persistent** test order for Payme's cabinet autotests: `npm run payme:test-order` (`-- --count 5`, `-- --list`, `-- --reset <id>`, `-- --cleanup`). Distinct from `payme:selftest`, which deletes its fixtures at the end — the cabinet's tests call us from outside, so the order must still exist when they run. Each successful sandbox run leaves the order `CONFIRMED`, i.e. spent; `--reset` drops its `PaymeTransaction`/`Payment` rows and returns it to `PENDING`.
...
- **Only provider:** Payme. Test cashbox: `https://test.paycom.uz`; production: `https://checkout.paycom.uz` (`PAYME_CHECKOUT_URL`). The checkout endpoint takes **no request body at all** — no `provider`, no `lang`. The service always builds a Payme link, so accepting a `provider` advertised a choice that was silently ignored; the *response* still carries it. Adding Click later means re-adding an optional field, not a breaking change.
- **The cashier window's language comes from the buyer's profile**, not from the request: `createCheckout` reads `order.user.language` and passes it through `normalizeLanguage()` into the `l=` link parameter. `payme.receipt.ts` already derives fiscal item titles the same way, so cashier and receipt can no longer disagree. A client-supplied `lang` was a second, contradictable source of truth for a value already stored in the DB.
- **Two directions.** `GET /api/payments/checkout/:order_id` (JWT) only **builds a cashier link** — it never marks anything paid. Payme's own server then calls `POST /api/payments/payme`, and the order becomes `CONFIRMED` inside `PerformTransaction`. The previous `payOrder` flipped payments to `SUCCESSFUL` on the spot, so orders counted as paid without any money moving.
...
- **Fiscal receipt (Uzbek OFD):** `CheckPerformTransaction` must return `result.detail` alongside `allow: true`, or Payme cannot fiscalize the payment. [payme.receipt.ts](file:///home/baxa/coding/e-commerse-back/src/api/payments/payme/payme.receipt.ts) builds it from the order's items: `code` (MXIK/IKPU), `package_code`, `vat_percent`, `units`, `count`, `discount`, and `price` **in tiyin**. **IKPU belongs to a product *group*, so its home is `Category`** (`ikpu_code`, `package_code`, `vat_percent`, `units`) — filling 8 category rows covers all 54 products. `Product` carries the same four fields as an **override**, needed because one category ("Инструменты") holds two unrelated groups: welding machines and electric motors. Resolution is `Product → Category → throw`. The old `PAYME_DEFAULT_*` env fallback is **gone**: a single code applied to the whole catalog, so a voltage stabilizer was fiscalized as a pump. `code` and `vat_percent` are **required** by Payme — both throw `MissingFiscalDataError` → `-31008` when absent, rather than letting Payme reject the receipt mid-payment; note `vat_percent` resolves with `??` not `||`, since `0` is a real value (not VAT-registered). `package_code` and `units` are omitted entirely when unset — sending `units: 0` or `package_code: ""` would be an invalid classifier code. `onModuleInit` queries the DB and warns at boot about categories missing fiscal data (errors swallowed — a cold DB must not block startup). Item titles use the **buyer's** language (`order.user.language`) — Payme's request carries no language.
